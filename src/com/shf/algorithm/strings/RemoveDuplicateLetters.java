...
//            要求只有当前字母比已经找到的position位置的字母要小，才有资格继续判断
...
//        遍历结束，position位置的字母就是结果中最左侧的元素
        return s.charAt(position) + removeDuplicateLetters1(
...
//        递归调用
        return s.charAt(position) + removeDuplicateLetters2(
                s.substring(position+1).replaceAll(s.charAt(position)+"",""));
    }
...
//        为了快速判断一个字符串是否在某个位置之后重复出现，用一个HashMap来保存字母出现在字符串的最后位置
...
//                c入栈之前，要判断之前栈顶元素，是否在后面重复出现；如果重复出现就可以删除
                if (!stack.isEmpty() && c < stack.peek() && lastOccur.get(stack.peek()) > i) {
...
//        将栈中的元素保存成字符串输出
    ...
    public static void main(String[] args) {
        ...
        System.out.println(removeDuplicateLetters.removeDuplicateLetters3(str1));
        System.out.println(removeDuplicateLetters.removeDuplicateLetters3(str2));
    }
}
